...
raw_data = df.to_numpy() #just had to change, get_values doesn't exist anymore
...
#we don't need to do what's written above because last column is already 1 or 0 for present or absent
#however family history in X is present or absent so we need to change that

familyhistLabels = raw_data[:,5]
familyhistNames = np.unique(familyhistLabels)
familyhistDict = dict(zip(familyhistNames,range(len(familyhistNames))))

for i in range(len(familyhistLabels)) :
    X[i][4] = familyhistDict[familyhistLabels[i]]
    
    
#problem X is an array of object and a array of float would be better
...
y = raw_data[:,-1] #last column
classDict = {'absent' : 0, 'present' :1} #creating the name of the class and linked them to a number in y
classNames = np.array(['absent','present'])
